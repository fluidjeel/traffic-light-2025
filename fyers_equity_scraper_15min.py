# fyers_equity_scraper_15min.py
#
# Description:
# This script is a modified version of the original daily scraper, specifically
# tweaked to download and update historical 15-MINUTE data for all stocks
# in the Nifty 200 index. It fetches data in smaller, 90-day batches
# to handle the stricter API limits for intraday data.
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Error: 'nifty200.csv' not found.")
        sys.exit()

    print(f"\nFound {len(symbols)} stocks to process for 15-minute data.")

    for i, symbol_name in enumerate(symbols):
        print(f"\nProcessing {symbol_name} ({i+1}/{len(symbols)})...")
        output_path = os.path.join(output_dir, f"{symbol_name}_15min.csv")
        ...
        while batch_start_date <= to_date.date():
            ...
            batch_df = get_historical_data(fyers_symbol, "15", from_date_str, to_date_str)
    ...
    print("\n--- 15-minute data update complete! ---")
