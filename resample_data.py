...
# 'fyers_equity_scraper.py' script and converts them into 2-day, weekly,
# and monthly data files.
#
# How to use:
# 1. First, run 'fyers_equity_scraper.py' to download the daily data.
#    This will create a 'historical_data' folder.
# 2. Then, run this script. It will create three new folders:
#    - '2day_data': containing the resampled 2-day data.
...
def resample_historical_data():
    ...
    for i, filename in enumerate(daily_files):
        ...
        try:
            ...
            # --- Resample to 2-Day Data ---
            twoday_df = daily_df.resample('2D').apply(ohlc_logic).dropna()
            if not twoday_df.empty:
                twoday_output_path = os.path.join(twoday_output_dir, f"{symbol_name}_2day.csv")
                twoday_df.to_csv(twoday_output_path)
                print(f"  > Success: Saved 2-day data to {twoday_output_path}")
            else:
                print("  > Info: No 2-day data to save.")
