# simulator_fast_entry_price_confirm.py
#
# Description:
# Implements Strategy 2: Enters trades on intraday price confirmation ONLY.
# It does not use any EOD or intraday filters for entry.
# It now performs a continuous, 15-minute reconciliation check on all trades
# entered today.
# - If a trade fails the check and is profitable, SL is moved to breakeven.
# - If a trade fails the check and is losing, the position is closed.
#
# MODIFICATION:
# 1. Changed the intraday entry trigger from candle 'high' to candle 'close'
#    to require a stronger confirmation for entry.
...
config = {
    ...
    # NEW: Fixed log folder
    'simulator_log_folder': 'fast_entry_price_confirm_logs',
    ...
    # --- FILTERS (DISABLED FOR ENTRY) ---
    'intraday_market_strength_filter': False, # DISABLED
    'volume_velocity_filter': False,          # DISABLED
    'market_regime_filter': False,            # DISABLED for watchlist
    'volume_filter': False,                   # DISABLED for watchlist
    'rs_filter': False,                       # DISABLED for watchlist

    # --- BENCHMARK FILTERS (USED ONLY FOR RECONCILIATION) ---
    'benchmark_regime_index_symbol': 'NIFTY200_INDEX',
    'benchmark_regime_ma_period': 50,
    'benchmark_volume_ma_period': 20,
    'benchmark_volume_multiplier': 1.3,

    # --- NEW RECONCILIATION PARAMETERS ---
...
def run_backtest(cfg):
    ...
    index_df_daily = None
    symbols_to_load = symbols + [cfg['benchmark_regime_index_symbol']]
        ...
        try:
            ...
            if os.path.exists(daily_file):
                ...
                if not df_d.empty and all(col in df_d.columns for col in ['open', 'close']):
                    ...
                    if cfg['benchmark_regime_index_symbol'] in symbol:
                        index_df_daily = df_d
                else:
    ...
    print("Starting Strategy 2: Fast Entry with Price Confirmation...")
    for date in master_dates:
        ...
        # --- Intraday Loop ---
        try:
            today_intraday_candles = intraday_data.get(cfg['benchmark_regime_index_symbol'], pd.DataFrame()).loc[date.date().strftime('%Y-%m-%d')]
        except KeyError: 
        ...
        if not today_intraday_candles.empty:
            ...
            for candle_time in today_intraday_candles.index:
                ...
                # --- Simplified Intraday Entry Logic (Price Only) ---
                for symbol, details in list(todays_watchlist.items()):
                    ...
                    # *** LOGIC CHANGE: Entry now on candle CLOSE above trigger ***
                ...
                for pos_id, pos in list(portfolio['positions'].items()):
                    # DEFINITIVE FIX:
                    # 1. Only check positions entered today.
                    # 2. Only check positions that have NOT been reconciled yet.
                    # 3. DO NOT check a position in the same candle it was opened.
                    if (pos['entry_date'].date() != date.date() or 
                        pos.get('recon_managed', False) or 
                        pos['entry_date'] == candle_time):
                        continue
                    
                    symbol = pos['symbol']
                    
                    # --- Check against BENCHMARK EOD filters ---
                    meets_benchmark_criteria = True
                    try:
                        # 1. Market Regime
                        if index_df_daily.loc[date]['close'] < index_df_daily.loc[date][f"ema_{cfg['benchmark_regime_ma_period']}"]:
                            meets_benchmark_criteria = False
                        ...
                        avg_daily_volume = daily_data[symbol].iloc[prev_day_loc][f"volume_{cfg['benchmark_volume_ma_period']}_sma"]
                        if current_day_volume < (avg_daily_volume * cfg['benchmark_volume_multiplier']):
                            meets_benchmark_criteria = False
                        ...
                        temp_index_close = index_df_daily['close'].loc[:date - timedelta(days=1)].append(pd.Series([intraday_data[cfg['benchmark_regime_index_symbol']].loc[candle_time]['close']], index=[date]))
                    ...
                    if not meets_benchmark_criteria:
                        pos['recon_managed'] = True # Mark as checked for today
                        current_price = intraday_data[symbol].loc[candle_time]['close']
                        if current_price > pos['entry_price']: # Profitable
        ...
        # --- EOD Watchlist Generation (Pattern Only) ---
    ...
    for index, log_entry in all_setups_df.iterrows():
        ...
        entry_date_str = log_entry['setup_date']
        entry_date = pd.to_datetime(entry_date_str).tz_localize(None).to_pydatetime() + timedelta(days=1)
        ...
        if entry_date in daily_data.get(symbol, pd.DataFrame()).index:
            try:
                ...
                continue # Skip if entry_date is not in index
    ...
    summary_content = f"""BACKTEST SUMMARY REPORT (STRATEGY 2: FAST ENTRY PRICE CONFIRM)
===================================================================
{params_str}
