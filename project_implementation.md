...
Post-Hoc Analysis: The mae_analyzer_percent.py script ingests the detailed trade logs generated by the simulators to perform "what-if" simulations for data-driven stop-loss optimization.
...
After Market Close on Friday (The "Scout" Mission): The Scout scans the weekly charts for the core pullback pattern. For each valid setup, it generates a "Target List" for the entire upcoming week.

During the Following Week, Monday to Friday (The "Sniper" Mission): The Sniper monitors the 15-minute data for stocks on the Target List. When a breakout occurs, it validates the move using the full Advanced Conviction & Risk Engine before entering.
...
Phase 3.5 (Next): Advanced Strategy Analysis & Optimization.
