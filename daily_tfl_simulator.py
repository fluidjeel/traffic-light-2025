...
def run_backtest(cfg):
    ...
    pipeline_cfg = cfg['data_pipeline_config']
    if pipeline_cfg['use_universal_pipeline']:
        print("--- Using UNIVERSAL Data Pipeline ---")
        cfg['data_folder_base'] = pipeline_cfg['universal_processed_folder']
        cfg['intraday_data_folder'] = pipeline_cfg['universal_intraday_folder']
        intraday_index_filename_format = "{symbol}_15min.csv"
    else:
        print("--- Using LEGACY Data Pipeline ---")
        cfg['data_folder_base'] = pipeline_cfg['legacy_processed_folder']
        cfg['intraday_data_folder'] = pipeline_cfg['legacy_intraday_folder']
    ...
    for date in master_dates:
        ...
        if not today_intraday_candles.empty and todays_watchlist:
            ...
            for candle_idx, (candle_time, mkt_candle) in enumerate(today_intraday_candles.iterrows()):
