...
#
# v28 (Calculation Integrity Fixes):
# - Fixed risk calculation to account for slippage on both entry and exit.
# - Added logic to handle gap-down exits at the open price.
# - Corrected high/low watermark initialization to include the entry day's candle.
# - Changed breakeven buffer to a more logical absolute calculation.
...
config = {
    ...
    'strategy_name': 'multi_candle_breakout_fixed', # A name for the strategy run, used for creating log folders.
    'nifty_list_csv': 'nifty500.csv', # The CSV file containing the universe of stock symbols to trade.
    'breakout_lookback_period': 2, # The number of days to look back for the breakout high/low. Set to 1 for original strategy.
...
def run_portfolio_backtest(cfg):
    ...
    for i, date in enumerate(master_dates):
        ...
        # --- 1. EOD Trailing Stop Update (Using T-1 Data) ---
        for symbol, pos in portfolio['positions'].items():
            if symbol in all_data and date in all_data[symbol].index:
                ...
                # ATR Trail calculation using T-1 data
        ...
        # --- 2. Check for Stop-Loss Hits & Update Watermarks ---
        positions_to_close = []
        for symbol, pos in list(portfolio['positions'].items()):
            ...
            # INTEGRITY FIX: Check for gap down exit first
        ...
        for symbol, order in todays_watchlist.items():
            ...
            if symbol not in all_data or date not in all_data[symbol].index:
                ...
                if today_candle['high'] < order['trigger_price']:
                    ...
                    # INTEGRITY FIX: Calculate risk with slippage on both sides
                    ...
                    if risk_per_share <= (entry_price * (ef_cfg['min_risk_percent'] / 100)):
                        ...
                        capital_at_risk = eod_equity_yesterday * (current_risk_percent / 100) * risk_multiplier
            ...
            # INTEGRITY FIX: Initialize watermarks correctly including entry day's candle
        ...
        # --- 5. Scan for New Setups for Next Day ---
