...
#
# v22 Update (Worst-Case Optimization):
# - To improve performance during difficult market periods (like 2025), the
#   Volume Filter has been significantly tightened.
# - The 'min_volume_ratio' has been increased from 1.5 to 3.0 to only allow
#   the highest-conviction, highest-volume breakouts to be traded.
...
config = {
    ...
    'allow_shorting': False,         # This strategy is long-only. Set to True to enable shorting logic (not implemented).
    'setup_candle_type': 'both',     # Can be 'green', 'red', or 'both'. 'both' was found to be most effective.
    ...
    'portfolio_risk_management': {
        'use_max_portfolio_risk_cap': False, # If True, uses 'max_portfolio_risk_percent' instead of 'max_open_positions'.
        'max_portfolio_risk_percent': 13.0,  # The maximum total risk allowed across all open positions.
    },
    ...
    'trade_management': {
        ...
        'atr_multiplier': 8.0,  # The default ATR multiplier if the dynamic exit is disabled.
...
def run_portfolio_backtest(cfg):
    ...
    prm_cfg = cfg['portfolio_risk_management']
    ...
    for i, date in enumerate(master_dates):
        ...
        for symbol, order in todays_watchlist.items():
            ...
            if prm_cfg['use_max_portfolio_risk_cap']:
                total_current_risk = sum(p.get('capital_at_risk', 0) for p in portfolio['positions'].values())
                max_allowed_portfolio_risk = eod_equity_yesterday * (prm_cfg['max_portfolio_risk_percent'] / 100)
                potential_new_risk = eod_equity_yesterday * (current_risk_percent / 100) * risk_multiplier # Apply multiplier
                if total_current_risk + potential_new_risk > max_allowed_portfolio_risk:
                    rejection_reason = "Entry Rejected: Max portfolio risk cap would be exceeded"
            else: 
            ...
            portfolio['positions'][symbol] = {
                ...
                'setup_candle_type': order['setup_candle_type'], 'setup_candle_date': order['setup_candle_date'],
            ...
            if symbol in all_data and date in all_data[symbol].index:
                ...
                if date > pos['entry_date'] and tm_cfg.get('use_atr'):
                    ...
                    if prev_day_index >= 0:
                        ...
                        pos['stop_loss'] = max(pos['stop_loss'], pos['highest_price_since_entry'] - (atr_val * atr_mult))
        ...
        if i + 1 < len(master_dates) and scan_for_setups:
            ...
            for symbol in symbols:
                ...
                if is_valid_setup:
                    watchlist[next_day][symbol] = {'direction': 'long', 'trigger_price': setup_candle['high'], 'stop_loss_price': setup_candle['low'], 'setup_candle_type': 'Red' if setup_candle.get('close') < setup_candle.get('open') else 'Green', 'setup_candle_date': date}
