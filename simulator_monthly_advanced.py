...
#
# MODIFICATION (v5.1 - Final Audit Corrections):
# 1. FIXED: VIX Lookahead Bias. The intraday logic now uses the VIX close
#    from two days prior (T-2) to ensure all data is historically available
#    at the time of a trade decision on day T.
# 2. REINSTATED: Slippage Model. A realistic slippage percentage, which is
#    adaptive to VIX levels, is now applied to all entry prices.
# 3. REINSTATED: Conviction Filters. All original EMA, volume, and market
#    strength filters have been added back to the config and logic. They are
#    disabled by default to allow for methodical research.
# 4. REINSTATED: Partial Profit-Taking. The 'use_partial_profit_leg' feature
#    has been fully restored to the exit logic.
...
config = {
    ...
    'timeframe': 'monthly',
    'use_ema_filter': False, # Reinstated - enable for research
    'ema_period_monthly': 10,
    'use_monthly_volume_filter': False, # Reinstated - enable for research
    ...
    'market_strength_threshold_calm': -0.25, # Reinstated
    'market_strength_threshold_high': -0.75, # Reinstated
    'base_slippage_percent': 0.10, # Reinstated
    'high_vol_slippage_percent': 0.20, # Reinstated
    'use_market_strength_filter': False, # Reinstated - enable for research

    # --- Trade Management & Profit Taking ---
    'profit_target_mode': 'RISK_BASED',
    'use_partial_profit_leg': False, # Reinstated - enable for research
...
class MonthlyAdvancedSimulator:
    ...
    def sniper_monitor_and_execute(self, date, cash_at_sod):
        ...
        try:
            ...
            if vix_loc < 2: return # Not enough historical VIX data
                ...
                if stock_candle['high'].item() >= details['trigger_price']:
                    ...
                    if self.cfg.get('use_market_strength_filter', False):
                        ...
                        if mkt_strength < threshold:
                            continue # Skip trade if market is weak
                    # --- END REINSTATED ---

                    self.execute_entry(symbol, details, candle_time, vix_close_t2, cash_at_sod)
                    todays_new_positions += 1
                    if symbol in todays_watchlist: del todays_watchlist[symbol]
    
    def execute_entry(self, symbol, details, candle_time, vix_close, cash_at_sod):
        entry_price_base = details['trigger_price']
        
        # --- REINSTATED: Slippage Model ---
        ...
        for pos_id, pos in list(self.portfolio['positions'].items()):
            try:
                ...
                if 'target' in pos and candle['high'].item() >= pos['target']:
                    # --- REINSTATED: Partial Profit Logic ---
